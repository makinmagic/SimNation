...
async function displayLotInfo(lotId) {
  ...
  <p><strong>Roommates:</strong> ${
    roommateNames.length > 0
      ? roommateNames.map(name => `<span style="color: #4FC3F7;">${name}</span>`).join(', ')
      : 'None'
  }</p>
    ...
    knownSims.length > 0
      ? knownSims.map(name => {
          ...
          if (trimmed === ownerName) {
            ...
            return `<span style="color: #4FC3F7;">${trimmed}</span>`;
          } else {
            return trimmed;
          }
        }).join(', ')
...
async function fetchPlayerImages() {
    const playerImagesUrl = 'https://makinmagic.github.io/SimNation/profile-pictures.json';
    try {
...
async function displayPlayerInfo(avatarId) {
    ...
    try {
        ...
        const playerImage = playerImages[simName] || 'https://makinmagic.github.io/SimNation/images/pfp-placeholder.png';
...
function toggleSearch(type) {
    ...
    if (simSearchInput.style.display === 'none') {
        ...
        document.getElementById('console-content').innerHTML = '<p style="text-align: center; color: #E7E7E7;">Select or search for a Sim or Lot to see more information, or click on an Event to view details.</p>';
    }
}
...
async function searchSim(event) {
    if (event.key === 'Enter' || event.keyCode === 13) {
        ...
        try {
            ...
            // Match the Sim name to the JSON file
            const playerImage = playerImages[playerData.name] || 'https://makinmagic.github.io/SimNation/images/pfp-placeholder.png';
...
const eventsUrl = 'https://makinmagic.github.io/SimNation/events.json';
...
async function fetchMoneyObject() {
    ...
    const response = await fetch(`https://makinmagic.github.io/SimNation/data/highest_paying_object.json${cacheBuster}`);
    const data = await response.json();
